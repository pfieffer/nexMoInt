...
package np.com.ravi.nexmoint.Country;
...
import com.nexmo.sdk.verify.ui.countriesAdapter.Country;
    ...
    public CountryList(final Context context) {
        ...
        if (countryCodes.length == countryNames.length)
            for (int i = 0; i < countryCodes.length; i++)
                this.countries.add(new Country(countryNames[i], countryCodes[i], countryPrefixes[i])); //new Country(countryNames[i], countryCodes[i])
        else
